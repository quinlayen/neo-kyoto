class TransitSignals:
    ...
    def submit_report(self):
        if not all(s == "FIXED" for s in self.signals.values()):
            print(f"    Cannot submit: not all signals fixed yet.")
            return
        self.report_submitted = True
        print(f"    Report submitted. All signals verified and logged.")

    def is_goal_met(self):
        return all(s == "FIXED" for s in self.signals.values()) and self.report_submitted

    def get_status_text(self):
        fixed = sum(1 for s in self.signals.values() if s == "FIXED")
        total = len(self.signals)
        report = "SUBMITTED" if self.report_submitted else "PENDING"
        indicator = "[OK]" if self.is_goal_met() else "[!!]"
        return f"""  TRANSIT HUB — SIGNAL CONTROL
  Status:     {indicator} {fixed}/{total} signals fixed
  Report:     {report}
"""
