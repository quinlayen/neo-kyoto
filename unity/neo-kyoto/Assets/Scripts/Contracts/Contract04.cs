...
{
    public class Contract04 : Contract
    {
        public override string Id { get { return "contract_04"; } }
        public override string Title { get { return "Signal Interference"; } }
        public override string Location { get { return "Transit Hub"; } }
        public override string ScriptName { get { return "signals.py"; } }
        public override int MaxCalls { get { return 25; } }

        public TransitSignals Signals { get; private set; }

        public Contract04() { ResetSystem(); }

        public override void ResetSystem()
        {
            Signals = new TransitSignals();
            Signals.Output = Print;
            Signals.Changed += RaiseSystemChanged;
            RaiseSystemChanged();
        }
        ...
        public override Dictionary<string, CommandFunc> GetCommands()
        {
            ...
            cmds["check_signal"] = delegate (List<object> args)
            {
                return Signals.CheckSignal(ArgInt(args, 0, "check_signal(1)"));
            };
            cmds["reset_signal"] = delegate (List<object> args)
            {
                return Signals.ResetSignal(ArgInt(args, 0, "reset_signal(1)"));
            };
            cmds["calibrate_signal"] = delegate (List<object> args)
            {
                return Signals.CalibrateSignal(ArgInt(args, 0, "calibrate_signal(1)"));
            };
            cmds["submit_report"] = delegate (List<object> args) { return Signals.SubmitReport(); };
            return cmds;
        }
        ...
        public override string StarterScript
        {
            get
            {
                return @"# ── Transit Hub Signals ─────────────────────────
#
# Your available commands:
#   check_signal(n)      — shows the signal's state
...
";
        ...
        {
            return "◆ CONTRACT #2480 COMPLETE — Transit Signals Fixed ◆\n" +
